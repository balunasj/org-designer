#!/usr/bin/env tsx
/**
 * Import script: reads data/all_users.json + org repo YAML → data/baseline.json
 *
 * Run with:  npm run import   (or: make import)
 * Requires:  data/all_users.json  (generate with: make fetch-users)
 *
 * Override source paths via env vars:
 *   ALL_USERS_PATH=/path/to/all_users.json npm run import
 *   ORG_FLEET_PATH=/path/to/fleet npm run import
 *
...
// Prefer data/all_users.json (generated by make fetch-users) then fall back to
// the fastrover repo copy so existing setups keep working.
const LOCAL_ALL_USERS = path.join(DATA_DIR, 'all_users.json')
const FASTROVER_FALLBACK = path.join(process.env.HOME!, 'Projects/fastrover/all_users.json')
const FASTROVER_PATH =
  process.env.ALL_USERS_PATH ??
...
console.log(`Loading user data from ${FASTROVER_PATH}...`)
